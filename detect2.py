...
#     sum_activations = np.sum(feature_map, axis=(1, 2));print(sum_activations.shape)
#     eps = 0.000001
...
#     plt.clf()
#     plt.bar(x,y)
#     plt.savefig(f'{out_name}_plot.png', dpi=200)
 

    heatmap = cv2.applyColorMap(np.uint8(255 * cam), cv2.COLORMAP_JET)
    cam_img = 0.3 * heatmap + 0.7 * img
    
    #cv2.imwrite(f'{out_name}_{i:03d}.jpg', cam_img)
    cv2.imwrite(f'{out_name}.jpg', cam_img)
...
def run(weights='yolov5s.pt',  # model.pt path(s)
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        t1 = time_sync()
        if pt:
            visualize = increment_path(save_dir, mkdir=True, exist_ok=True) if visualize else False
            ...
            if visualize:
                ...
                for i in [21, 24, 27, 30]:
                    out_name = str(visualize / f"{Path(path).stem}_{i}")
